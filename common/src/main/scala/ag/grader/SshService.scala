...
class SshException(user: Maybe[String], host: Maybe[String], port: Maybe[Int], extra_flags: Seq[String], cause: Exception) extends Exception(cause) {
...
trait SshService {
  ...
  def run(parts: os.Shellable*)(using World): (os.Path, os.Path) throws SshException = {
    run_as()(parts: _*)
  }
}
  ...
  override def run_as(
    ...
    cd(os.pwd) {
      try {
        ...
        case e: ShellException => throw SshException(user, host, port, extra_flags, e) 
      }
    }
  }
}
